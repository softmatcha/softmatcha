...
class TestSearchNaive:
    def test_search_fullmatch_glove(
        ...
        searcher = SearchNaive(pattern, tokenizer_glove, embed_glove)
    ...
    def test_search_fullmatch_bert(
        ...
        text_tokens = tokenizer_bert("Soft pattern match.")
        text_embeddings = embed_bert(text_tokens)
        pattern = Pattern.build(
            text_tokens, text_embeddings, [1 - 1e-5] * len(text_embeddings)
        )
        searcher = SearchNaive(pattern, tokenizer_bert, embed_bert)
        text = TokenEmbeddings(text_tokens, text_embeddings)
        res = searcher.search(text)
        assert next(res)
        with pytest.raises(StopIteration):
    ...
    def test_search_semantic_match_bert(
        ...
        pattern = Pattern.build(
            pattern_tokens, pattern_embeddings, [0.6] * len(pattern_embeddings)
        )
        searcher = SearchNaive(pattern, tokenizer_bert, embed_bert)
        text = TokenEmbeddings(text_tokens, text_embeddings)
        res = searcher.search(text)
    ...
    def test_search_semantic_no_match_glove(
        ...
        pattern_embeddings = embed_glove(pattern_tokens)
        pattern = Pattern.build(
            pattern_tokens, pattern_embeddings, [0.75] * len(pattern_embeddings)
        )
        searcher = SearchNaive(pattern, tokenizer_glove, embed_glove)
        text = TokenEmbeddings(text_tokens, text_embeddings)
        res = searcher.search(text)
